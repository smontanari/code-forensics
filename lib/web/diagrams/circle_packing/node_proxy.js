...
module.exports = function(config) {
  ...
  this.nodeVisible = function(weightThreshold, valueThreshold, node) {
    var isRootNode = _.isUndefined(node.parent);
    var isNodeAboveWeightThreshold = _.isUndefined(node[config.series.calculatedWeightProperty]) || _.isNumber(node[config.series.calculatedWeightProperty]) && node[config.series.calculatedWeightProperty] >= weightThreshold;
    var isNodeAboveValueThreshold = _.isNumber(node.value) && node.value >= valueThreshold;

    return isRootNode || (isNodeAboveWeightThreshold && isNodeAboveValueThreshold);
  };
