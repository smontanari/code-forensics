...
module.exports = function(options) {
  ...
  this.nodeVisible = function(node) {
    return self.nodeHiglighted(node) ||
      ...
      node[seriesConfig.calculatedWeightProperty] >= options.weightFilter.changeValue()) &&
      node.value >= options.valueFilter.changeValue();
  };
  ...
  this.circleNodeFill = function(node) {
    ...
    return node[seriesConfig.calculatedWeightProperty] > 0.0 ? styleConfig.colorValues.weightColor : node.children ? styleConfig.colorScale(node.depth) : styleConfig.colorValues.noColor;
  };
