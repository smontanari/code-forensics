...
module.exports = function(config, series) {
  ...
  var seriesGroups = _.groupBy(_.flatMap(series, 'keyDefinitions'), 'group');
  var seriesNames = _.compact(_.map(series, 'name'));
  var colorScale = ColorScaleFactory.default(seriesNames);
  ...
  if (series.length > 1) {
    legendDefinition = new LegendModel(config, series, colorScale).legendDefinition;
  }
  ...
  this.chartDefinitions = [
    {
      name: 'main',
      properties: {
        attributes: { width: chartConfig.width, height: chartConfig.height },
      },
      components: [
        {
          name: 'xAxis',
