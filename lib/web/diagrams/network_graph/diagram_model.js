...
module.exports = function(configuration, data) {
  var allValues = _.uniqBy(_.map(data.nodes, 'team'));
  var colorScale = configuration.colorScaleFactory(allValues);
  ...
  this.graphData = data;
  this.colorMap = _.map(allValues, function(name) {
  ...
  this.chartDefinitions = [
    {
      name: 'main',
      properties: {
        attributes: { width: configuration.style.width, height: configuration.style.height }
      },
      components: [
        {
          name: 'link-data',
          ...
          graphicElements: [
            {
              ...
              tooltip: {
                properties: {
                  attributes: { class: 'd3-tip network-graph-diagram link-info' },
                  html: function(link) {
          ...
          graphicElements: [
            {
              ...
              tooltip: {
                properties: {
                  attributes: { class: 'd3-tip network-graph-diagram node-info' },
                  html: function(node) {
                    return mustacheHelper.renderTemplate.apply(null, [
