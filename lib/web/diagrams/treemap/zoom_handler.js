...
module.exports = function(nodeHelper) {
  ...
  var prepareDataForTransition = function(node) {
    var components = diagramModel.createDataDefinition(node);

    var dataComponent = _.find(components, { 'name': 'treemap-container' });
    var dataContainer = new D3Data(mainSvgDocument, dataComponent).element;
