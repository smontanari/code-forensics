...
module.exports = function(configuration, nodesArray) {
  ...
  var colorScale = configuration.colorScaleFactory(nodeNames);
  ...
  this.colorMap = _.map(nodeNames, function(nodeName) {
    return { name: nodeName, color: colorScale(nodeName) };
  });
  ...
  this.chartDefinitions = [
    {
      name: 'header',
      properties: {
        attributes: { width: configuration.style.width, height: configuration.style.margin.top }
      },
      components: [
        {
          name: 'root-tile',
    ...
    },
    {
      name: 'main',
      properties: {
        attributes: { width: configuration.style.width, height: height }
      },
