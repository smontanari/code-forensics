...
module.exports = function(config, series) {
  ...
  this.height = d3.max([series.length/config.visibleBars * height, height]) - margin;

  var seriesValues = _.map(this.visibleSeries(), config.valueProperty);
  ...
  this.colorScale = function(d) { return colorScaleFn(d[config.valueProperty]); };

  this.axis = {
    x: {
      translate: 'translate(0,' + margin + ')',
      scale: function(d) { return xScale(d[config.valueProperty]); },
      value: d3.svg.axis()
        .orient('top')
        .scale(xScale)
        .outerTickSize(2)
