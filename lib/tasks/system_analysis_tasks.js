...
module.exports = function(context, taskDef) {
  ...
  taskDef.add("system-evolution-analysis", '\nUsage: system-evolution-analysis --boundary <b> [--dateFrom <date> --dateTo <date> --frequency <freq>]', ['vcs-log-dump', 'generate-boundaries-file'], function() {
    var runAnalysis = function(analyser, reportFile, transform) {
      ...
      .mergeAll(utils.functions.arrayToFnFactory(context.timePeriods, function(period) {
        return codeMaat[analyser]
        .fileAnalysisStream(context.files.temp.vcslog(period), ['-i', '1', '-n', '1', '-m', '1', '-g', context.files.temp.codeBoundaries()])
    ...
    return reportHelper.publish('system-evolution', function(publisher) {
