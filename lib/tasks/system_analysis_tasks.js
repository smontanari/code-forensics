  ...
  taskDef.addAnalysisTask('system-evolution-analysis',
    {
      ...
      parameters: [{ name: 'dateFrom' }, { name: 'dateTo' }, { name: 'timeSplit' }, { name: 'boundary' }],
      reportFiles: {
    ...
    var groupParam = _.isEmpty(context.boundaries) ? {} : { '-g': helpers.files.codeBoundaries() };

    var runAnalysis = function(analysis, reportFile, streamTransform) {
    ...
    if (_.isEmpty(context.boundaries)) {
      return Q.allSettled(_.map([
        ...
        [
          'entityChurnAnalysis',
          publisher.addReportFileForType('churn-trend'),
          function(period) {
            return reduce.obj(function(previous, obj) {
              return _.assign(previous, {
