...
module.exports = function(taskDef, context, helpers) {
  ...
  taskDef.addAnalysisTask('developers-coupling-analysis',
    ...
    var authorsCommunicationAnalysis = function() {
      var outputStream = utils.json.objectToFileStream(
        publisher.addReportFileForType('communication-map'),
        codeMaat.communicationAnalyser()
        .fileAnalysisStream(helpers.files.vcslog(context.dateRange))
        .pipe(reduce.obj(function(array, obj) {
          if (!_.find(array, { 'author': obj.coupledAuthor, 'coupledAuthor': obj.author })) {
            array.push(obj);
          }
          return array;
        }, [])));

      return utils.stream.streamToPromise(outputStream);
    };

    return Q.allSettled([authorsCouplingAnalysis(), authorsCommunicationAnalysis()]);
  });

  taskDef.addAnalysisTask('knowledge-map-analysis',
    {
      description: 'Analyse the distribution and the loss of knowledge amongst developers for each file',
