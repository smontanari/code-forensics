...
module.exports = function(context, taskDef) {
  ...
  var produceHotspotReport = function() {
    var dataSources = [
      ReportComposer.newDataSource(context.files.temp.revisions(), pathMatchers.haveSamePath, 'revisions')
    ].concat(_.map(_.filter(
      _.map(context.languages, context.files.temp.codeComplexity),
      utils.fileSystem.isFile
    ), function(file) {
      return ReportComposer.newDataSource(file, pathMatchers.haveSamePath, 'totalComplexity');
    }));
    ...
    return reportHelper.publish('hotspot-analysis', function(publisher) {
      return new ReportComposer(context.files.temp.sloc())
        ...
        .buildReport().then(function(reportData) {
          var dataTree = graphDataHelper.weightedTree(reportData, 'path', 'revisions');
