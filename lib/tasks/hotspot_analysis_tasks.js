...
module.exports = function(taskDef, context, helpers) {
  taskDef.add('revisions-analysis', 'Report on the number of vcs revisions for each file\nUsage: gulp revisions-analysis [--dateFrom <date> --dateTo <date>]', ['vcs-log-dump'], function() {
    return codeMaat.revisionsAnalyser.fileAnalysisStream(helpers.filesHelper.vcslog(context.dateRange))
    .pipe(filter.obj(function(obj) { return context.repository.isValidPath(obj.path); }))
    .pipe(utils.json.objectArrayToFileStream(helpers.filesHelper.revisions()));
  });

  var produceHotspotReport = function() {
    var dataSources = [
      reporting.ReportComposer.newDataSource(helpers.filesHelper.revisions(), {
      ...
      _.map(context.languages, helpers.filesHelper.codeComplexity),
    ...
    return helpers.reportHelper.publish('hotspot-analysis', function(publisher) {
      return new reporting.ReportComposer(helpers.filesHelper.sloc())
